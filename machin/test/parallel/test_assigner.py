...
from machin.machin.parallel.assigner import ModelSizeEstimator, ModelAssigner
from machin.machin.utils.helper_classes import Object
...
class TestModelSizeEstimator:
    def test_estimator(self):
        ...
        model.register_parameter(
            "param1", nn.Parameter(t.zeros([10], dtype=t.int8), requires_grad=False)
        )
        model.register_parameter(
            "param2", nn.Parameter(t.zeros([10], dtype=t.int16), requires_grad=False)
        )
        model.register_parameter(
            "param3", nn.Parameter(t.zeros([10], dtype=t.int32), requires_grad=False)
        )
        model.register_parameter(
            "param4", nn.Parameter(t.zeros([10], dtype=t.double), requires_grad=False)
        )
        model.register_buffer("buffer1", t.zeros([10], dtype=t.float))
        assert ModelSizeEstimator(model, size_multiplier=1).get_buffer_sizes() == (
            10 * 4
        ) / (1024 ** 2)
        assert ModelSizeEstimator(model, size_multiplier=1).get_parameter_sizes() == (
            10 * 1 + 10 * 2 + 10 * 4 + 10 * 8
        ) / (1024 ** 2)
        assert ModelSizeEstimator(model, size_multiplier=1).estimate_size() == (
...
class TestModelAssigner:
    ...
    def test_assigner(self, params, gpus, cpu, assignment, exception, match):
        ...
        with mock.patch(
            "machin.parallel.assigner.GPUtil.getAvailable",
            self.patch_gputil_get_available,
        ) as _p1, mock.patch(
            "machin.parallel.assigner.GPUtil.getGPUs", self.patch_gputil_get_gpus
        ) as _p2, mock.patch(
            "machin.parallel.assigner.psutil.virtual_memory",
            self.patch_psutil_virtual_memory,
        ) as _p3, mock.patch(
            "machin.parallel.assigner.ModelSizeEstimator",
            self.patch_model_size_estimator,
        ) as _p4:
