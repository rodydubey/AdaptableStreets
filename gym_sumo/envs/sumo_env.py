...
class SUMOEnv(Env):
	...
	def __init__(self,discrete_act=False,reset_callback=None, reward_callback=None,
	...
	def getState(self,agent_name):
		...
		if agent_name == "agent 0": # car
			...
			# state_2 = self.traci.lanearea.getLastIntervalMeanSpeed('det_2')	
			# state_3 = self.traci.lanearea.getLastIntervalOccupancy('det_2')
			# state_4 = self.traci.lanearea.getLastIntervalOccupancy('det_0')	
			# state_5 = self.traci.lanearea.getLastIntervalOccupancy('det_1')
		...
		if agent_name == "agent 2": 
			...
			# xx = self.traci.lanearea.getLastIntervalOccupancy('det_0')	
			# yy = self.traci.lanearea.getLastIntervalOccupancy('det_1')
			# ll = self.traci.lanearea.getLastIntervalMeanSpeed('det_0')	
			# mm = self.traci.lanearea.getLastIntervalMeanSpeed('det_1')
			# aa = self.traci.lanearea.getLastIntervalVehicleNumber('det_0')
			# bb = self.traci.lanearea.getLastIntervalVehicleNumber('det_1')
			if aa + bb:
	...
	def reset(self,scenario):		
		...
		if scenario=="Train":
			self._slotId = np.random.randint(1,37)
	...
	def collectObservation(self):
		...
		if laneWidthBike == 0:
			...
			self.traci.lane.setDisallowed('E0_0',disallowed3)
			allowed = []
			allowed.append('bicycle')
			allowed.append('pedestrian')        
			self.traci.lane.setAllowed('E0_0',allowed)
			self.traci.lane.setDisallowed('E0_1', ["all"])			
		else: 
			...
			self.traci.lane.setDisallowed('E0_1',disallowed)
			self.traci.lane.setAllowed('E0_1','bicycle')
			...
			self.traci.lane.setDisallowed('E0_0',disallowed2)
			self.traci.lane.setAllowed('E0_0','pedestrian')
		...
		self._total_waiting_time_bike += self.traci.lane.getWaitingTime('E0_1') 
		self._total_waiting_time_ped += self.traci.lane.getWaitingTime('E0_0') 
		# test stats
		...
		if cosharing:
			...
			if (self._sumo_step % 10 == 0) and self._scenario=="Test":
	...
	def _step(self,action_n):
		...
		print("Reward = " + str(reward_n), sum(reward_n), 'Cosharing', cosharing)
	...
	def _set_action(self, actionDict, action_space, time=None):
		# process action
		t = 0
		adaptNetwork(self.base_netfile,actionDict,self._routeFileName,self.sumoCMD, self.pid, self.traci, self.discrete_act)
		# t = 0
	...
	def advanceTimeSlot(self):
		...
		# self._slotId = 290
		self._routeFileName = "testcase_0/two/intersection_Slot_" + str(self._slotId) + ".rou.xml"
		print('Advancing Timeslot:', self._routeFileName)
		self.timeOfHour +=1
